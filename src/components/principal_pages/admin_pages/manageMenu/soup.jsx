...
function Soups(props){
        ...
        /*Sección de estados*/
        const [soup, setSoups] = useState([]); /*Aquí se almacenan las sopas creadas en la bd*/
        const [updateItems, setUpdateItems] = useState(false); /*Este estado se usa para el useEffect para el momento en que se agrego una sopas a la bd*/
        ...
        /*Función que trae solo las sopas de la bd*/
        const menu_data = async () => {
          try {
            ...
            if (data.status === 200) {
              ...
              const sopasFilter = results.filter(
                (item) => item.menu_item_type === "soups"
              );
              setSoups(sopasFilter);
            }
          } catch (error) {
        ...
        const sendForm = (e) => {
          ...
          formData.append("menu_item_type", "soups");
        ...
        /*Cuando se agrega un nuevo elemento a las sopas, se actualizan con este hook*/
        useEffect(() => {
          menu_data();
        ...
        /*Esta función es la que selecciona de las sopas, que elementos que van a ir en el menú del día*/
        const selectItem = async (id, name) => {
          const selectedSpeciality = soup.find((items) => items.id === id);
          try {
                  ...
                  fetch(url, {
                    ...
                    .then((data) => {
                      if (data.status === 200) {
                        setUpdateItems(true);
                        props.setChangeFather(true);
                        toast.success(name + " se eliminó de las sopas");
                      }
                    });
                },
              },
    ...
    return(
        ...
        <>
          <>
            {soup.length !== 0 ? (
              <>
                <h4>Seleccione las sopas para hoy </h4>
              ...
              <>
                <div className="especialitiesEmpty">
                  <h4>
                    Aún no hay sopas creadas, ingresa una en el siguiente
            ...
            <div className="formEspecialitiesContainer">
              ...
              >
                <div className="mb-3">
                  <label htmlFor="name">Nombre de la sopa</label>
                  <Input
                    value={name}
                    onChange={(e) => {
                ...
                <div className="mb-3">
                  ...
                  <Textarea
                    ...
                    id="description"
                    rows="3"
                    placeholder="Describa brevemente que incluye o de que trata la sopa"
                  ></Textarea>
                </div>
                <div className="mb-3">
