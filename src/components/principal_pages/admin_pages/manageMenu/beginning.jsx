...
function Beginning(props){
    ...
    /*Sección de estados*/
    const [beginning, setBeginning] = useState([]); /*Aquí se almacenan las especialidades creadas en la bd*/
    ...
    const menu_data = async () => {
      try {
        ...
        if (data.status === 200) {
          ...
          const beginningFilter = results.filter(
            (item) => item.menu_item_type === "beginning"
          );
    ...
    const sendForm = (e) => {
      ...
      formData.append("menu_item_type", "beginning");
    ...
    const selectItem = async (id, name) => {
      const selectedSpeciality = beginning.find((items) => items.id === id);
      try {
              ...
              fetch(url, {
                ...
                .then((data) => {
                  if (data.status === 200) {
                    setUpdateItems(true);
                    props.setChangeFather(true);
                    toast.success(name + " se eliminó de los principios");
                  }
                });
            },
          },
...
return(
    ...
    <>
      <>
        {beginning.length !== 0 ? (
          <>
            <h4>Seleccione los principios para hoy </h4>
          ...
          <>
            <div className="especialitiesEmpty">
              <h4>
                Aún no hay principios creados, ingresa una en el siguiente
        ...
        <div className="formEspecialitiesContainer">
          ...
          >
            <div className="mb-3">
              <label htmlFor="name">Nombre de la principio</label>
              <Input
                value={name}
                onChange={(e) => {
            ...
            <div className="mb-3">
              ...
              <Textarea
                ...
                id="description"
                rows="3"
                placeholder="Describa brevemente que incluye o de que trata el principios"
              ></Textarea>
            </div>
            <div className="mb-3">
